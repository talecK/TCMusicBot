...
class ServerDataAccess(object):
    ...
    def set_currently_playing(self, song=None):
        ...
        if isinstance(song, Song):
            return self.storage.use_collection('server_stats').update({}, {'currently_playing': extract_song_data(song), 'status': 'playing'})
        else:
            return self.storage.use_collection('server_stats').update({}, {'currently_playing': {}, 'status': 'polling'})
